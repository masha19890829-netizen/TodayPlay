...
- 模拟器安装截图 QA 未完成。当前本机模拟器 adb 连接反复掉线，并且系统 `package` 服务返回 `DEAD_OBJECT / Can't find service: package`，无法稳定安装或截图。这是设备环境阻塞，不是构建失败。
...
- 换一台稳定模拟器或真机完成安装、冷启动、首页、生成、结果页、小屏、折叠屏截图。
